...
class TestStore:
    ...
    def test_create(self, testapp):
        res = testapp.post_json('/v3/stores', {
            'name': 'foo',
            'address': 'Toshima',
        })
        data = res.json
    ...
    def test_fetch(self, testapp):
        ...
        # error
        # not found
        res = testapp.get('/v3/stores/%d' % 99,
                          expect_errors=True,
                          )
        data = res.json
    ...
    def test_update(self, testapp):
        new_name = 'foo'
        res = testapp.patch_json('/v3/stores/1', {
            'name': new_name,
        })
        data = res.json
        ...
        # not found
        res = testapp.patch_json('/v3/stores/31', {
            'name': 'bar',
        },
            expect_errors=True,
        )
    ...
    def test_delete(self, testapp):
        ...
        # delete
        res = testapp.delete('/v3/stores/%d' % store_id)

        assert res.status_int == 204
        assert res.text == ''

        # check
        res = testapp.get('/v3/stores/%d' % store_id,
                          expect_errors=True,
                          )
        ...
        res = testapp.delete('/v3/stores/%d' % 1,
                             expect_errors=True,
                             )
        data = res.json
    ...
    def test_search(self, testapp):
        res = testapp.post_json('/v3/search/stores', {
            'address': 'Shibuya',
        })
        data = res.json
